...
export const AdminView: React.FC<AdminViewProps> = ({ onReturnToSite }) => {
  ...
  const handleLogin = (e: React.FormEvent) => {
    e.preventDefault();
    const rawAlias = loginAlias.trim();
    const rawPassword = loginPassword.trim();

    // Requisito: alias IDEenseñanza y contraseña institutoIDE
    const isTargetAlias =
      rawAlias === 'IDEenseñanza' ||
      rawAlias.toLowerCase() === 'ideenseñanza' ||
      rawAlias.toLowerCase() === 'ideensenanza';

    const isTargetPassword = rawPassword === 'institutoIDE';

    if (isTargetAlias && isTargetPassword) {
    ...
    // Verificar si coincide con otro usuario activo registrado
    const matchedUser = data.users.find(
      u =>
        (u.alias && u.alias.toLowerCase() === rawAlias.toLowerCase()) ||
        (u.email && u.email.toLowerCase() === rawAlias.toLowerCase())
    );

    if (matchedUser && matchedUser.active && rawPassword === 'institutoIDE') {
      setCurrentUser(matchedUser);
      setLoginError(null);
      return;
    }

    setLoginError('Alias o contraseña incorrectos. Verifique sus credenciales de acceso.');
  };
        ...
        <div className="w-full max-w-md bg-slate-900 border border-slate-800 rounded-3xl p-8 shadow-2xl space-y-6">
          ...
          <form onSubmit={handleLogin} className="space-y-4">
            <div>
              <label className="block text-xs font-bold text-slate-300 uppercase tracking-wider mb-1.5 flex items-center justify-between">
                <span>Alias</span>
                <span className="text-[11px] text-slate-500 font-normal lowercase">usuario de acceso</span>
              </label>
              <div className="relative">
                <input
                  ...
                  required
                  placeholder="IDEenseñanza"
                  value={loginAlias}
                  onChange={(e) => setLoginAlias(e.target.value)}
                  autoComplete="username"
                  className="w-full bg-slate-800 border border-slate-700 text-white placeholder-slate-500 focus:border-blue-500 px-3.5 py-2.5 rounded-xl text-sm outline-none transition-colors"
                />
              </div>
            </div>
            ...
            <div>
              <label className="block text-xs font-bold text-slate-300 uppercase tracking-wider mb-1.5 flex items-center justify-between">
                ...
                <button
                  ...
                  className="text-[11px] text-blue-400 hover:text-blue-300 font-normal cursor-pointer flex items-center gap-1"
                >
            ...
            <button
              ...
              className="w-full py-3 bg-blue-600 hover:bg-blue-500 active:bg-blue-700 text-white font-bold text-xs uppercase tracking-wider rounded-xl shadow transition-colors cursor-pointer flex items-center justify-center gap-2"
            >
          ...
          <div className="pt-2 border-t border-slate-800 text-center">
            <span className="text-[11px] text-slate-500">
              Acceso exclusivo para personal directivo y administrativo autorizado
            </span>
          </div>
              ...
              {editingUser && (
                <div className="bg-white p-6 rounded-2xl border-2 border-blue-500 shadow-xl space-y-4">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                    ...
                    <div>
                      <label className="block text-xs font-bold text-slate-700 mb-1">Alias de Acceso</label>
                      <input
                        type="text"
                        placeholder="Ej: IDEenseñanza"
